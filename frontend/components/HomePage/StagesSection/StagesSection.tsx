...
const CARD_OFFSET_X = 220; // расстояние между карточками
...
const CardStack: React.FC = () => {
  ...
  return (
    <div className={styles.mobileSwiperWrap}>
      <ul className={styles.cardWrapStyle}>
        {steps.map((card, index) => {
          const isActive = index === activeIndex;

          // Смещение карточек в зависимости от их позиции относительно активной
          ...
          // z-index: активная всегда сверху
          ...
          return (
            <motion.li
              ...
              onDragEnd={(event, info) => {
                if (info.offset.x < -50 && activeIndex < steps.length - 1) {
                  moveToNext();
...
export default function StagesSection() {
  return (
    <section className={styles.stages}>
          ...
          <div className={styles.grid}>
            {steps.map(({ title, number, description, image }) => (
              <div key={number} className={styles.step}>
                <Image alt={title} src={image} width={138} height={144} className={styles.step_number} />
                <div>
      ...
      {/* --- MOBILE: кастомный CardStack вместо swiper --- */}
      <div className={styles.mobileVersion}>
        <Image
          alt={'Этапы сотрудничества'}
          src={'/home_page/stages_photo.png'}
          width={900}
          height={1200}
          className={styles.stages_photo_mobile}
        />

        <Htag color={'gradient'} tag={'h2'} uppercase={true} className={styles.mobileTitle}>
            Этапы производства
        </Htag>
